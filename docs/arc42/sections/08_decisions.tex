...
\begin{tabularx}{\textwidth}{|L{2.5cm}|X|}
\hline
\textbf{Context} & Need to collect metrics from servers \\
\hline
\textbf{Alternatives} & Pull-based (server scrapes targets like Prometheus) \\
\hline
\textbf{Decision} & Push-based collection where agents POST metrics to API \\
\hline
\textbf{Rationale} & Pull requires inbound network access to monitored servers. Push works through firewalls and NAT without opening ports. Agents can batch and retry on failure. \\
\hline
\textbf{Consequences} & + Works through firewalls, no inbound ports needed \newline + Agents control send frequency \newline + Easier to add new servers (no central config) \newline - Server must handle burst traffic \\
\hline
\end{tabularx}
...
% ============================================================================
\subsection{ADR-008: Server-Sent Events for Dashboard}
\label{subsec:adr008}
...
\begin{tabularx}{\textwidth}{|L{2.5cm}|X|}
\hline
\textbf{Context} & Dashboard needs real-time metric updates without constant polling \\
\hline
\textbf{Alternatives} & Polling (client requests every N seconds), WebSocket (bidirectional), SSE (server-to-client push) \\
\hline
\textbf{Decision} & Server-Sent Events (SSE) for pushing metrics and alerts to dashboard \\
\hline
\textbf{Rationale} & Dashboard only needs one-way server-to-client push. SSE is simpler than WebSocket, has automatic reconnection, works over standard HTTP, and is sufficient for our use case. WebSocket would add complexity for bidirectional capability we don't need. \\
\hline
\textbf{Consequences} & + Simpler than WebSocket \newline + Automatic reconnection built into browser API \newline + Works with standard HTTP infrastructure \newline - Cannot send client-to-server messages (not needed) \\
\hline
\end{tabularx}
\caption{ADR-008: Server-Sent Events for Dashboard}
\label{tab:adr008}
\end{table}
